...
function pickUpBox(box) {
    ...
    let distance = calculateDistance(player.position, box.position);
  
    // Check if the player is close enough to the box
    if (distance < threshold) {
      // Add the box to the player's inventory
      player.inventory.add(box);
  
      // Update the UI to show that the player has picked up the box
      updateInventoryUI(player.inventory);
  
      // Remove the box from the game world
      removeBoxFromGame(box);
    }
  }
